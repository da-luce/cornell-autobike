...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument("topic", description="a pointcloud topic to process", default_value="/pointcloud"),
        Node(
            package='pointcloud_to_grid',
            executable='grid_trajectory',
            output='screen',
            parameters=[
                {'cloud_in_topic': '/pointcloud'},
                {'position_x': -20.0},  # meters
                {'position_y': 0.0},  # meters
                {'verbose1': False},
                {'verbose2': False},
                {'cell_size': 0.2},  # meters
                {'length_x': 50.0},  # meters
                {'length_y': 100.0},  # meters
